...
`data.frame()`. It would be inconvenient if 
...
Second, the author may want to allow other packages to use ____. 
...
function just for that class, the package's creator can make a version of the 
plot function that will display objects of their class. The 
`sf` class in the sf package is a good example of this. Objects of the `sf`
class represent geographical information, like the spatial extent of the 
...
the option to select documentation for the plot function in the sf package.
If we do that, there are many more arguments listed in the Usage and Arguments
sections. In particular, we can specify the position of the key (legend), plot
the data on a log scale, and add a title. 
