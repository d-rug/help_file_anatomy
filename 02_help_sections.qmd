...
all of the named arguments listed out. Arguments with an `=` after them have a default
value. This means you don't necessarily need to provide a value for that argument when calling the function. However it is important to check that the default
value is what you want it to be. If there is no `=` after
...
<!-- igraph for networks, can modify network node size, shape and color, and  -->
<!--   label location, as well as connecting edge linewidth, and arrow size -->
<!-- terra and sf for maps, change color scheme, zoom level of map, scale, what  -->
<!--   variable gets visualized on the map -->
<!-- stats plot diagnostics for different types of statistical models -->
...
is the type of data we use can to make maps.
...
and nothing seems to work. The examples can also help you understand how a function expects the input data to be structured.
